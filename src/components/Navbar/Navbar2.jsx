...
import { FaSignInAlt, FaClipboardList, FaReply } from "react-icons/fa";
    ...
    >
      <div>
        ...
        >
          ...
          <div className={`border-l-4 border-b-4 border-solid border-white bg-colorBodies3 transition-all md:w-10/12 w-full sm:flex hidden items-center md:static absolute px-10 md:py-5 text-white rounded-bl-full`}>
            {navLinks.map((navLink) => {
              return <NavLink key={navLink.id} {...navLink} />;
            })}
            <li className='list-none cursor-pointer space-x-2'>
              <a href="/" className="p-2 font-bold text-sm border-2 border-solid rounded-lg border-white hover:bg-white hover:text-colorBodies3 inline-flex items-center">
                <FaReply className="mr-2"/> Kembali
              </a>
            </li>
          </div>
